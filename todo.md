...
- [ ] `tpt-e-link` core types implemented and buildable: `LinkMessage`, `DeviceManifest` (wire
...
- [ ] Decide dependency strategy for `tpt-e-link` in `tpt-e-node`'s `Cargo.toml`: path
...
- [ ] Create Cargo workspace root (`Cargo.toml`) with members `crates/tpt-node-core`,
      `crates/tpt-node-esp32`, `crates/tpt-node-riscv`.
- [ ] Exclude `templates/*` from the workspace (they're `cargo-generate` templates, not build
      members).
- [ ] Add `tpt-e-link` as a path dependency of `tpt-node-core`.
- [ ] Build the Reactor / event-loop skeleton in `tpt-node-core` using `embassy-executor`
      (lightweight `no_std` async executor), listening on `LinkTransport` for incoming `Command`s.
- [ ] Implement the Capability Registry API:
      `node.register_capability(Capability::LittleFS(my_fs_driver))`, etc.
- [ ] Implement Command dispatch: route incoming `Command` payloads to registered capability
      handlers.
- [ ] Define the local `Capability` enum aligned with `tpt-basestation`'s `DeviceCapability`
...
- [ ] Internal handler: if `Capability::OTA` is registered, `tpt-node-core` auto-handles chunked
      download, CRC verification, and flash writing.
- [ ] Unit tests for reactor/registry against `tpt-e-link`'s mock transport (`std` feature).
...
- [ ] `tpt-node-core` constructs the wire-format `DeviceManifest` populated with registered
      capabilities and broadcasts it on boot.
- [ ] *(tpt-basestation repo)* Add a mapping function: wire `DeviceManifest` →
      `hal::DeviceManifest` projection.
- [ ] *(tpt-basestation repo)* Extend the discovery pipeline (`hal/src/discovery/`) to accept a
      tpt-e-link handshake as a new discovery method alongside the existing serial regex probe.
- [ ] Document the field mapping between `tpt-e-link::DeviceManifest` and
...
- [ ] `tpt-node-mock`: `std` Rust binary implementing `LinkTransport` over a local WebSocket
      client.
- [ ] Register fake capabilities in `tpt-node-mock`, including a simulated LittleFS backed by
      the host's local hard drive.
- [ ] *(tpt-basestation repo)* Add a WebSocket listener in `tpt-core` accepting tpt-e-link-framed
      messages from `tpt-node-mock` (new transport — today only serial/BLE/HTTP-OTA exist).
- [ ] *(tpt-basestation repo)* Wire WebSocket-discovered devices into the existing
      `DeviceCardDeck` / capability-gated UI (`gui/src/cards/registry.tsx`).
- [ ] End-to-end check: `tpt-node-mock` boots, handshakes over WebSocket, and BaseStation's UI
...
- `tpt-basestation/hal/src/manifest.rs`, `hal/src/capability.rs`, `hal/src/discovery/` —
  existing manifest/capability/discovery system to integrate with (Phase 2)
- `tpt-basestation/core/src/serial/`, `core/src/ble/`, `core/src/ota.rs` — existing transports;
  WebSocket is new surface area (Phase 3)
- `tpt-basestation/gui/src/cards/registry.tsx`, `gui/src/App.tsx` — existing capability-gated
  UI to wire new devices into (Phase 3)
- `tpt-embedded-core/crates/tpt-e-typestate-hal` (+ `tpt-e-chronos`, `tpt-e-cipher`,
  `tpt-e-slumber`) — HAL/primitives to depend on for `tpt-node-esp32` (Phase 4)
