//! `tpt-node-core` — the architecture-agnostic runtime and state machines for
//! the TPT Embedded Node ecosystem.
//!
//! This crate provides the "brains" of a TPT-capable device: a capability
//! registry, command dispatch, an internal OTA handler, and an async reactor
//! loop that drives a [`tpt_e_link::LinkTransport`].
//!
//! It is `#![no_std]` + `alloc`, designed to run on bare-metal microcontrollers
//! (ESP32, RISC-V) under `embassy-executor`, while remaining testable on the
//! host via the `std` feature and `tpt-e-link`'s mock transport.
//!
...
pub use capability::{
    cmd, Capability, CapabilityRegistry, CommandHandler, LittleFsDriver, TelemetryDriver,
};
pub use error::NodeError;
pub use ota::{FlashWriter, OtaHandler, RamFlashWriter};
pub use reactor::Node;
